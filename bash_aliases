...
Install () { [ "$#" -eq 0 ] && echo "Usage: Install <keyword or package(s)>" && return; if [ $# -eq 1 ]; then pikaur -Sslq $1 | sort -u | fzf -q $1 -i -m --reverse --preview 'pikaur -Sii {1}' --preview-window right:60%:wrap | xargs -r pikaur -S --needed --noedit; else pikaur -S --needed --noedit $@; fi; }
# https://wiki.archlinux.org/title/Pacman/Tips_and_tricks#Packages_and_dependencies
Purge	() { if [ "$#" -eq 0 ]; then comm -23 <((pacman -Qqen; pacman -Qqm)| sort) <((expac -l '\n' '%E' base-devel; expac -l '\n' '%E' base) | sort -u) | sort -u | fzf -i -m --reverse --preview 'pikaur -Sii {1}' --preview-window right:80%:wrap | xargs -r pikaur -Rsc; else pikaur -Rsc $@; fi; }
