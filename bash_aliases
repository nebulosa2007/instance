...
if [ "$(mount | grep -o ' / type btrfs')" != "" ]; then
    alias snapctl="\ls -d /.snapshots/*/ | sed 's/\/.snapshots\///g;s/\/\$//g' | fzf -m --reverse --preview '$INSTANCESCRIPTWAY/snaplist.sh {1}' --preview-window right:70%:wrap | xargs -I SNAP sudo btrfs subvolume delete /.snapshots/SNAP"
