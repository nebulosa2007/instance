...
#SYSTEM MAINTAINING
getnews () { echo -ne '\033[0;34m:: \033[0m\033[1mRequired by: '; echo -e '\033[0m'; max=$(pacman -Qqu | wc -L); for pkg in $(pacman -Qqu); do printf "%*s:%s\n" $max $pkg "$(pacman -Qi "$pkg" | grep Req | sed -e 's/Required By     : //g')" | column -c85 -s: -t -W2; done; echo; echo -ne '\033[0;34m:: \033[0m\033[1mMirror: '; grep -m1 Server /etc/pacman.d/mirrorlist | cut  -d'/' -f3; echo -e '\033[0m'; pikaur -Syu --noedit; }
