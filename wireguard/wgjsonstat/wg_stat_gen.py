...
def wg_json_traffic_data(wg_ui_path):
    try:
...
if __name__ == "__main__":
    wg_ui_path = "/opt/wireguard-ui/db/"
    wg_json_path = "/home/ds/.config/instance/wireguard/wgjsonstat/"
    wg_json = wg_json_traffic_data(wg_json_path)
    clients = []
    clients_files = [_ for _ in glob(wg_ui_path + "clients/*.json")]
    ...
    for _ in range(len(clients_id)):
        ...
        try:
            ...
            tstat = []
        try:
            transferRx = wg_json['wg0']['peers'][client['public_key']]['transferRx']
            transferTx = wg_json['wg0']['peers'][client['public_key']]['transferTx']
            instant_traffic = {datetime.now().strftime(
                "%Y-%m-%d"): {'transferRx': transferRx, 'transferTx': transferRx}}
        except:
            instant_traffic = ''
        if instant_traffic:
            tstat.append(instant_traffic)
        for_stat_file = {client['public_key']: {
            "name": client['name'], "tstat": tstat}}
        try:
