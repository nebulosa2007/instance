...
# Credits: https://github.com/ngoduykhanh/wireguard-ui
...
sudo cp $PATHINSTANCE/wireguard/wgwebui/wgui-reload-conf.{service,path} -t /lib/systemd/system/
sudo cp $PATHINSTANCE/wireguard/wgwebui/wgui.service /lib/systemd/system/wgui.service
sudo cp $PATHINSTANCE/wireguard/wgwebui/wgiptablesrules.sh /opt/wireguard-ui/wgiptablesrules
sudo systemctl daemon-reload
sudo systemctl enable --now wgui-reload-conf.{service,path} {wgui,wg-quick@wg0}
# Then open in browser ip_server:5000
# Login and password is admin:admin. Login and change password for you own!!
...
# Status wgui-reload-conf.{service,path} {wgui,wg-quick@wg0}
